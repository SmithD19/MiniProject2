...
env2_names <-
  df %>% select(width, shaded, wtemp:ph, total_cover) %>% colnames()
...
aggregated1 <- df %>%
  ...
  summarise(
    ...
    exposure = round(mean(as.numeric(
...
aggregated1.1 <- df %>%
  ...
  group_by(site, plot, season) %>%
  # Sum for eac
  summarise_at(.vars = vars(pred_names),
               .funs = sum,
               na.rm = T) %>%
...
# Aggregated data for other structural factors
# Assuming the factors dont change for each dip point within a plot - reasonable
aggregated1.2 <- df %>%
  # Grouping
  group_by(site, plot, season) %>%
  # select factors
  select(management) %>%
  # a distinct 185 rows, can just cbind to data frames
  distinct()
...
binaryaggregate <-
  function(x) {
    as.factor(ifelse(sum(x, na.rm = T) > 0, 1, 0))
  }
...
aggregated2 <- df %>%
  ...
  summarise_at(.vars = vars(env_names),
               .funs = binaryaggregate) %>%
  # Ungroup to stop dplyr magic
  ungroup()


# Function to convert factors to numeric and sum them
remove_me_fun <- function(x) {
...
remove_me_cooccurr <- aggregated2 %>%
  ...
  summarise_at(env_names, remove_me_fun) %>%
...
aggregated <-
  # join the data for abundance and water data
  left_join(aggregated1, aggregated1.1, by = c("site", "plot", "season")) %>%
  # add management data
  left_join(aggregated1.2) %>%
  # add coocurrence data
  left_join(aggregated2)

# Pull the abundance variables for mosquitoes
abun <- aggregated %>% select(mosquito_names, pred_names)

# Pull covariates
env <- aggregated %>% select(env2_names, management, env_names)

# Get rid of any rows in abundance data that dont have variance (all 0)
indexes <- which(rowSums(abun) == 0)
# Do this for abundance and Environmental covariates
abun <- abun[-c(indexes), ]
env <- env[-c(indexes), ]
...
# Carry on using 1 latent variables
fit_agg <-
  gllvm(
    y = abun,
    X = env,
...
# correlation plot
cr <- gllvm::getResidualCor(fit_agg)
...
coefplot(fit_ant, cex.ylab = 0.7)
# Works so unsure of the issue. "Hess" is present in the fit_ant gllvm object but not mine
