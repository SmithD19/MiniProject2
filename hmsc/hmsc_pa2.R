...
# Models for PA with predators as probit predictors of presence -----------

# This is a full model with co-occuring predators
PA2_Full_PredPred <- Hmsc(
...
# Covariates Categorical
X.cat3 <- dfhmsc %>% select(management, season) %>% mutate_all(as.character) %>% as.matrix()

# Covariates Landcover
X.lan3 <- dfhmsc %>% select(arable_horticulture:freshwater) %>% as.matrix()
...
models <- list("PA2_Full_PredPred" = PA2_Full_PredPred,
               "PA2_Full_PredResp" = PA2_Full_PredResp)
...
for (i in seq_along(models)) {
  ...
  # Filename Saving
  filename = file.path("hmsc/pa2_models", paste0(names(models)[[i]], ".RData"))
  # Save file
